...
public class RevisionArchivos10 {
				...
				for (String documento : documentos) {
					try (PreparedStatement pstmt = connection.prepareStatement(query)) {
						...
						if (!documentoEncontrado) {
						    ...
						    cell.setCellValue("Documento no encontrado");

						    // Crear un estilo de celda para el fondo amarillo
				...
				for (int j = srcRow.getFirstCellNum(); j < srcRow.getLastCellNum(); j++) {
					...
					if (srcCell != null) {
						...
						case FORMULA:
							...
							break;
						case BLANK:
							destCell.setCellValue(""); // Esto "limpia" el contenido de la celda
							break;
						case ERROR:
